...
import WatchesAccessoriesPage from 'c/watchesAccessoriesPage';

describe('c-watches-accessories-page', () => {
    afterEach(() => {
    ...
    it('renders hero header and breadcrumb without errors', () => {
        const element = createElement('c-watches-accessories-page', {
            is: WatchesAccessoriesPage
        });
        document.body.appendChild(element);

        const title = element.shadowRoot.querySelector('.hero-title');
        expect(title).not.toBeNull();
        expect(title.textContent).toContain('Watches & Accessories');

        const breadcrumb = element.shadowRoot.querySelector('.breadcrumb-current');
        expect(breadcrumb).not.toBeNull();
        expect(breadcrumb.textContent).toBe('Watches & Accessories');
    });
    ...
    it('filters products by search input', async () => {
        ...
        expect(title.textContent).toContain('Chronograph');
    });
