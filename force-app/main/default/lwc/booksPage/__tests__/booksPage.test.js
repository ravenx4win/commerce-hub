...
import BooksPage from 'c/booksPage';

describe('c-books-page', () => {
    afterEach(() => {
    ...
    it('renders hero header and breadcrumb without errors', () => {
        const element = createElement('c-books-page', {
            is: BooksPage
        });
        ...
        expect(title.textContent).toContain('Books & Literature');

        const breadcrumb = element.shadowRoot.querySelector('.breadcrumb-current');
        expect(breadcrumb).not.toBeNull();
        expect(breadcrumb.textContent).toBe('Books');
    });
    ...
    it('filters products by search input', async () => {
        ...
        expect(title.textContent).toContain('Atomic Habits');
    });
