...
import FoodGroceryPage from 'c/foodGroceryPage';

describe('c-food-grocery-page', () => {
    afterEach(() => {
    ...
    it('renders hero header and breadcrumb without errors', () => {
        const element = createElement('c-food-grocery-page', {
            is: FoodGroceryPage
        });
        ...
        expect(title.textContent).toContain('Food & Grocery Essentials');

        const breadcrumb = element.shadowRoot.querySelector('.breadcrumb-current');
        expect(breadcrumb).not.toBeNull();
        expect(breadcrumb.textContent).toBe('Food & Grocery');
    });
    ...
    it('filters products by search input', async () => {
        ...
        expect(title.textContent).toContain('Olive Oil');
    });
