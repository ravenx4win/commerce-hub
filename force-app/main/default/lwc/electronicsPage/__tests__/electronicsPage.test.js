...
import ElectronicsPage from 'c/electronicsPage';

describe('c-electronics-page', () => {
    afterEach(() => {
    ...
    it('renders hero header and breadcrumb without errors', () => {
        const element = createElement('c-electronics-page', {
            is: ElectronicsPage
        });
        ...
        expect(title.textContent).toContain('Electronics & Gadgets');

        const breadcrumb = element.shadowRoot.querySelector('.breadcrumb-current');
        expect(breadcrumb).not.toBeNull();
        expect(breadcrumb.textContent).toBe('Electronics');
    });
    ...
    it('filters products by search input', async () => {
        ...
        expect(title.textContent).toContain('ProVision');
    });
