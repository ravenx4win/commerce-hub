...
import TravelLuggagePage from 'c/travelLuggagePage';

describe('c-travel-luggage-page', () => {
    afterEach(() => {
    ...
    it('renders hero header and breadcrumb without errors', () => {
        const element = createElement('c-travel-luggage-page', {
            is: TravelLuggagePage
        });
        document.body.appendChild(element);

        const title = element.shadowRoot.querySelector('.hero-title');
        expect(title).not.toBeNull();
        expect(title.textContent).toContain('Travel & Luggage');

        const breadcrumb = element.shadowRoot.querySelector('.breadcrumb-current');
        expect(breadcrumb).not.toBeNull();
        expect(breadcrumb.textContent).toBe('Travel & Luggage');
    });
    ...
    it('filters products by search input', async () => {
        ...
        expect(title.textContent).toContain('Backpack');
    });
